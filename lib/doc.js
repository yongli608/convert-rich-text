...
Doc.prototype.writeText = function(text, attrs) {
  ...
  intersection(this.formatOrder, Object.keys(attrs)).forEach(function(name) {
    ...
    if (format && format.type !== 'line') {
      node = self.applyFormat(node, format, attrs[name], attrs);
    }
  });
...
Doc.prototype.applyFormat = function(node, format, value, attrs) {
  if (format.tag) {
    if (format.type === 'line') {
  ...
  if (format.parentTag) {
    dom(node).wrap(this.document.createElement(format.parentTag));
    node = node.parentNode;
  ...
  if (typeof format.add === 'function') {
    node = format.add(node, value, dom, attrs);
  }
...
Doc.prototype.formatLine = function(attrs) {
  ...
  intersection(this.formatOrder, Object.keys(attrs)).forEach(function(name) {
    var format = self.formats[name];
    if (format && format.type === 'line') {
      line = self.applyFormat(line, format, attrs[name], attrs);
    }
  });
