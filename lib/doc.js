...
Doc.prototype.applyFormat = function(node, format, value) {
  if (format.tag) {
    if (format.type === 'line') {
      node = dom(node, this.document).switchTag(format.tag).get();
    } else {
      if (dom.VOID_TAGS[format.tag]) {
        node = dom(node, this.document).replaceWith(this.document.createElement(format.tag)).get();
      } else {
        node = dom(node, this.document).wrap(format.tag).get();
      }
    }
  }

  if (format.parentTag) {
    node = dom(node, this.document).wrap(format.parentTag).get();
    if (node.previousSibling && node.tagName === node.previousSibling.tagName) {
      dom(node, this.document.previousSibling).merge(node);
    }
    if (node.nextSibling && node.tagName === node.nextSibling.tagName) {
      dom(node, this.document).merge(node.nextSibling);
    }
  }

  if (format.attribute || format.class || format.style) {
    if (dom(node, this.document).isTextNode()) {
      node = dom(node, this.document).wrap(this.inlineTag).get();
    }
    if (format.attribute) {
  ...
  if (typeof format.add === 'function') {
    node = format.add(node, value, this.document, dom);
  }
